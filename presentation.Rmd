...
- $\mathbf{X}$ é a matriz de planejamento (fixa e com $n$ linhas), cujas colunas são transformações das variáveis explicativas

---

## Modelo linear simples homocedástico FICA???

- A média da resposta condicionada nas observações é uma função linear desta: $E(\mathbf{Y} | \mathbf{X}) = \beta_0 + \beta_1 \mathbf{x_1} + ... + \beta_m \mathbf{x}_m = \mathbf{X} \boldsymbol{\beta}$
...
2. Um preditor aditivo $\boldsymbol{\eta} = \eta(\mathbf{X}) = s_0 + \sum_{j=1}^{m} s_j(\mathbf{x}_j)$
3. Uma função de ligação $g$ tal que $E(\mathbf{Y} | \mathbf{X}) = \boldsymbol{\mu} = g^{-1}(\boldsymbol{\eta})$

- $\boldsymbol{\eta}$ é estimado através do algoritmo *Backfitting* (Retroajuste), que minimiza $E[(\mathbf{Y} - s_0 - \sum_{j=1}^{m} s_j(\mathbf{x}_j))^2]$ e supõe que $\boldsymbol{\eta} = s_0 + \sum_{j=1}^{m} s_j(\mathbf{x}_j) + \boldsymbol{\epsilon}$, com $\boldsymbol{\epsilon} \sim N_n(\mathbf{0}_{1 \times n}, \sigma^2 \mathbf{I}_n)$
...
- Observação: $J_k^\prime, J_k$ e $q_{j k}$ são determinados em função da quantidade e definição das matrizes de planejamento
