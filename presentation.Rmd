...
## Modelando localização e escala por GAMLSS (Modelo Gama)
...
$$
\begin{aligned} 
  \log(\boldsymbol{\mu}) &= \boldsymbol{\eta}_{1} = \beta_{0} + \beta_{1} \mathbf{x}_{1} + \beta_{2} \mathbf{x}_{2} + \beta_{3} \mathbf{x}_{3} + \beta_{4} \mathbf{x}_{4} \\ 
  \log(\boldsymbol{\sigma}) &= \boldsymbol{\eta}_{2} = \beta_{0} + \beta_{1} \mathbf{x}_{1} + \beta_{2} \mathbf{x}_{2} + \beta_{3} \mathbf{x}_{3} + \beta_{4} \mathbf{x}_{4}
\end{aligned}
$$
...
- Distribuição dos resíduos (quantílicos normalizados) com assimetria negativa no gráfico da densidade estimada
...
R2_normal <- Rsq(normal_model)
R2_gamma <- Rsq(gamma_model)
```
...
Apesar do ajuste do Modelo Gama não ter sido adequado, ele obteve uma performance melhor que o Modelo Normal nas métricas $R^2$, AIC e BIC
