...
- As variáveis explicativas são:
    - *area*: Área em metros quadrados dos imóveis
    - *yearc*: Ano de construção
    - *cheating*: Contém aquecedor? Sim (1) e não (0)
    - *location*: Localização: abaixo da média (1), média (2) ou acima da média (3)
...
$$
Y_{i} = \beta_{0} + \beta_{1} x_{i 1} + \beta_{1} x_{i 1} + \beta_{1} x_{i 1} + \beta_{r} x_{i r} + \epsilon_{i}, \epsilon_{i} \stackrel{\text{ind}}{\sim} \mathcal{N}\left(0, \sigma^{2}\right)
$$
...
```{r}
par(mfrow=c(2,2))
term.plot(m1, data = df, se = T, partial.resid = T,
...
par(mfrow=c(2,2))
term.plot(m2, data = df, se = T, partial.resid = T,
          ask = F, col.res = rgb(0.7, 0.7, 0.7,0.3))
```
